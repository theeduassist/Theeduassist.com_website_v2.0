...
console.log("Validating Content Clusters...");
const clusterFile = path.join(process.cwd(), 'src/data/contentClusters/phase5Part2Clusters.ts');
...
if (count !== 12) {
    console.error(`❌ Expected 12 articles defined in phase5Part2Clusters.ts, found ${count}`);
    hasErrors = true;
}

// Ensure supporting articles specify a pillarSlug
...
if (hasErrors) {
  console.error("❌ Content cluster validation failed.");
  process.exit(1);
}

console.log("✅ Content clusters validated successfully.");
process.exit(0);
